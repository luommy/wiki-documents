...
### 2026-06-22 · Phase 2 模板校准（Task 2.3 复盘）

经过 #1 weather-forecast 与 #6 metric_card 两个入门案例的实战验证，模板整体可用，但有 3 个观察点：

- **§4 设计权衡行数目标偏高**：spec 给的目标是 ~80 行，实际两个案例都自然落在 35-45 行（prose only，不含代码块）。原因是"决策 × 替代方案 × 取舍"结构紧凑，不像 §3 实现剖析那样有大量代码片段撑体积。**建议 v1.1 调整目标为 50-60 行**（避免作者人为注水）。
- **"≥2 张 mermaid"硬性要求略显机械**：两个案例的第二张 mermaid 都是 sequence diagram，加进去确实增强了"数据流"可视化，但写起来需要刻意构造场景。**保持现状**（强制约束有利于一致性），但 v1.1 可以提供 3-4 种 mermaid 类型菜单（架构图 / 时序图 / 数据流图 / 状态图）让作者按案例自然选择。
- **验收清单核查成本**：每篇案例的 7 项验收清单 + 5 条硬标准要做一轮 spec review，每轮约 5-10 分钟。对于 7 案例 + 8 子页 = 15 个产物，总成本约 2 小时。**可接受**，但 v1.1 可以考虑把验收清单做成 lint 规则（例如自动检查代码片段行数、深链接格式）。

### Phase 2 已交付产物

- `1-weather-forecast.md`（中英，~420 行）— commit `c2c0b10c` + `91f379ed`
- `6-metric-card-component.md`（中英，~477 行）— commit `a81fd14f`

两个案例均通过完整 spec review。模板无需修订，Phase 3 可按原模板执行。
