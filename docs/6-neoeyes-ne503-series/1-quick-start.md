...
- **监控场景**：选择能覆盖目标区域的位置，注意镜头视场角（广角端水平 44.5°，长焦端 14.5°）
- **环境要求**：IP66 防护，支持 -30°C ~ +60°C 工作温度，可户外安装
...
**Control 标签页**：提供镜头控制面板，包括光学变焦（1x ~ 2.88x）、手动/自动对焦、IR-CUT 滤光片切换（auto/day/night）和补光灯控制。变焦后点击自动对焦确保画面清晰。

![](https://resources.camthink.ai/wiki/img/neoeyes-ne503-series/quick-start/qs-media-control.png)

> 广角端（10.2mm）用于全景监控，长焦端（29.5mm）用于远距离识别（如车牌、人脸）。
...
NE503 内置 64GB eMMC 用于系统和应用数据。如需扩展存储，可通过 TF 卡（支持 UHS-I）或 M.2 SSD（PCIe 3.0 × 2 Lanes）扩展。建议在存储使用率超过 80% 时及时清理或扩展。
...
| RTSP 三码流 | `rtsp://<设备IP>:8554/third` | 供电 | PoE 802.3AT 或 DC 12V，&lt; 8W |
