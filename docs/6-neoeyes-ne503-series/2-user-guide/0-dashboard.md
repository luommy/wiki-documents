...
设备当前时间、**Uptime**（本次开机的连续运行时长）、**Temperature**（SoC 与板载两路温度）。温度是判断散热与负载是否正常的关键指标——SoC 持续超过 80°C 时需检查安装环境通风，或减少运行中的模型 / 应用。
...
| **Memory Usage** | 内存占用 / 总量 | 接近上限时容器可能因 OOM 被杀，留意异常重启的应用 |
| **Storage Usage** | 存储占用 / 总量 | 长期超过 80% 需清理（规划见[部署与运维](./5-deployment.md)） |
