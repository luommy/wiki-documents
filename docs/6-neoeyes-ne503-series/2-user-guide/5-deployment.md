...
- **录像存储**：当前固件不提供本机录像存储，录像由外部 NVR / VMS 拉流完成（见[视频集成](../4-application-guide/3-reference/4-video-integration.md)）。
- **命令行管理**：使用 `aipc-cli`，常用命令见[平台服务总览](../3-software-guide/4-platform-services.md#cli-工具)。
