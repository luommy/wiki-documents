---
description: NE503 接口板的外部接口、板载资源、调试接口和主要引脚定义，涵盖控制、通信与电源信号。
keywords: [NE503, STM32G0B0, 接口板, IO配置, 引脚定义, RS-485, IR-CUT, 硬件连接]
tags: [NE503, 接口板, IO配置, 硬件连接]
---

# Interface Board

接口板通过 MCU 管理外部 IO、电源和外设控制，并通过 UART0 与核心板通信。外部端子提供 PoE、Wiegand、报警和音频接线。
...
| # | 功能 | 说明 | 类别 |
|:--|:-----|:-----|:-----|
| 3 | IR-CUT 驱动 | 日夜滤光片切换 | 内部 |
| 4 | 光敏检测 | ADC 环境光采样 | 内部 |
| 5 | 红外灯板驱动 | 近/远红外 PWM | 内部 |
| 6 | LENS Driver | SPI 镜头驱动 | 内部 |
| 7 | 系统指示灯 | 蓝色状态 LED | 内部 |
| 8 | 温度传感器 | ADC 温度采样 | 内部 |
| 9 | 双向复位 | SoC 复位 MCU / MCU 复位核心板 | 内部 |
| 10 | RTC | VBAT 断电保持 | 内部 |
...
MCU 通过 PD8 控制核心处理板复位。反向地，SoC 也可通过 H_GPIO_18（SN74LVC1G14DCK）复位本 MCU（详见 [核心处理板](./1-core-board-connection.md)）。
