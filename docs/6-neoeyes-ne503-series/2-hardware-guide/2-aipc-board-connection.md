...
MCU SPI1 接口为已安装的镜头模组提供控制路径。本文的 AN41908A-VBA 条目对应已记录的板级驱动器件；产品可选 `AF Lens (44.5° HFOV)` 或 `Motorized Zoom (110° HFOV)`，实际镜头能力、驱动器件和固件控制项以具体 SKU/BOM 为准。SoC 侧 SPI CS1 也连接该驱动路径（默认控制路径），MCU 侧负责镜头归零与限位保护。
