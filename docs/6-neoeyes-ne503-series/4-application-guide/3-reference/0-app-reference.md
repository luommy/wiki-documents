...
本文档是 NE503 容器应用的完整配置参考，涵盖 `app.yaml` 全部字段定义、Dockerfile 编写模式、安全沙箱模型以及多容器架构。如需面向新手的分步教程，请参阅 [SDK 示例](./2-sdk-examples.md)。
...
spec:
  ...
  # ── 卷挂载 ───────────────────────────────────
  volumes:
    - host: /data/aipc/data/my_app      # 宿主机路径
...
| 字段 | 类型 | 必需 | 说明 |
|:---|:---|:---|:---|
| `host` | string | 是 | 宿主机路径（设备部署根为 `/data/aipc`；写 `/opt/aipc` 前缀时平台会自动重映射到实际部署根，兼容旧写法） |
