...
本文通过 4 个由浅入深的完整 mini-app，演示 `hailo_ipc_sdk` 的典型用法。SDK API 详情请参阅 [SDK 参考](./1-sdk-reference.md)，项目结构与构建部署流程请参阅 [应用参考](./0-app-reference.md)。
...
关键约定（推理须用 `sub` 流；模型名与权限须与设备一致并和 `app.py`/`app.yaml` 对应）详见 [Person Detection 教程 §4](../2-cookbook/1-person-detection.md#23-查询设备上的真实值)。
:::

## 1. 实时目标检测计数器

:::note 完整真机版
本例是最简的推理订阅范式。完整真机版（含模型/流发现、健康检查、优雅退出、Web 验收）见 [Person Detection 教程](../2-cookbook/1-person-detection.md)。
:::
...
所有 mini-app 的构建与部署流程与教程一致：用仓库自带的 `build.sh` 一键完成「复制 SDK → ARM64 buildx → save → 打包 `.aipc`」，再按 Web 控制台 / aipc-cli / HTTP 三种方式之一部署到设备。完整步骤见 [Hello World §3 构建镜像](../1-app-development/1-hello-world.md#3-构建镜像) 与 [§4 部署到设备](../1-app-development/1-hello-world.md#4-部署到设备)；构建部署报错见 [故障排查 FAQ](../../5-troubleshooting.md)。

### 5.2 日志、常见问题与调试

- **日志查看与启动验收**：见 [Hello World §5 启动与验收](../1-app-development/1-hello-world.md#5-启动与验收)；
- **常见报错排查**（推理失败、权限拒绝、OOM 等）：见 [故障排查 FAQ](../../5-troubleshooting.md)；
- **调试环境变量**（`DEBUG`、`LOG_LEVEL`）：见 [应用参考 §7 环境变量参考](./0-app-reference.md#7-环境变量参考)。

---

## 6. 相关文档

- [应用参考](./0-app-reference.md) -- 项目创建、app.yaml 配置、构建部署完整流程
- [SDK 参考](./1-sdk-reference.md) -- SDK 全部模块的 API 详细参考
