---
description: NeoEyes NE503 是一款基于 Hailo-15H SoC 的边缘 AI 计算平台，搭载 20 TOPS NPU，支持 4K 视频编码、容器化应用管理、Web 控制台及多模型并发推理，适用于智能安防、工业检测、AIoT 等场景。
...
- **边缘 AI 闭环决策**：20 TOPS NPU 本地算力支持多模型并发推理，在设备端直接完成图像分析、事件判断与结构化结果输出，无需回传云端即可实现前端智能闭环，适用于对实时性和数据隐私有高要求的场景。

- **专业 4K 成像**：Sony IMX678 传感器搭配 Gen2 AI-ISP 与电动变焦镜头，支持 4K 编码与 &lt;0.01 LUX 全彩夜视，在 HDR、低照和多模型并发推理之间取得平衡，兼顾广角监控与远端识别。

- **容器化应用平台**：基于 containerd 容器运行时，通过 OCI 镜像部署第三方 AI 应用，安全沙箱隔离确保应用间互不干扰。模型与业务应用可按平台规范独立部署和升级，算法公司、方案商和 OEM 厂商可基于统一底座快速定制行业方案，打破硬件供应商锁定。

- **可交付产品能力**：NE503 集成成像、推理、告警联动、协议输出与运维管理于一体，具备从开发验证到项目交付的完整能力。IP66 防护搭配 PoE 一线供电，配合 Web 管理控制台，可直接投入商业化部署。

- **全栈开放工具链**：提供 Python / C++ / Go 多语言 SDK、aipc-cli 命令行工具及 RESTful API，满足不同开发接入方式。模块化 HAL 架构解耦平台服务与底层硬件，支持跨 SoC 平台平滑迁移，降低长期维护成本。
