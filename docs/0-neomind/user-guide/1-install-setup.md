...
安装后首次启动，NeoMind 会进入**配置向导**，引导你完成：

1. **创建管理员账号** — 设置用户名与密码（首次访问凭据）
2. **配置 LLM 后端** — 选择 Ollama（本地）或云端模型，详见 [配置 LLM 后端](./2-configure-llm.md)
3. **连接设备** — 进入 onboarding 向导，详见 [接入设备](./3-onboard-device.md)
...
`docker-compose.yml` 已包含后端、扩展运行器与前端，数据通过 volume 持久化。部署后访问 `http://host:9375`。
...
无论哪种安装方式，首次访问 Web UI 都要完成以下三步：

1. **创建管理员账号** — 首个注册的用户自动成为管理员
2. **配置 LLM 后端** — 让 AI Agent 能理解与执行指令，详见 [配置 LLM 后端](./2-configure-llm.md)
3. **接入设备** — 通过 onboarding 向导连接相机或传感器，详见 [接入设备](./3-onboard-device.md)
