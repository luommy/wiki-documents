...
```
[Source: bundle.js L239-L456](https://github.com/camthink-ai/NeoMind-Dashboard-Components/blob/main/components/ne101_camera/bundle.js#L239-L456)。Boa 的限制（无 `console.log`、不完整的 ES5 shim）在 8.3 单独复盘；本节聚焦 Node.js 侧的 `test_bundle.js`。这套双轨测试策略的价值在 7.3 ROI 矩阵里体现得最明显：纯函数 `computeOvTf` / `mapBbox` 在 Node 里验证数学正确性，生成出的 Transform JS 在 Boa 里跑实际推理。
...
```
[Source: bundle.js L879-L899](https://github.com/camthink-ai/NeoMind-Dashboard-Components/blob/main/components/ne101_camera/bundle.js#L879-L899)，它把归一化的检测框坐标映射到浏览器容器的屏幕像素，让 SVG `<rect>` 叠加在 `<img>` 上的位置与裁剪算法的判断一致。如果两者不一致，用户会看到「检测框明明在 ROI 多边形外面却被标红」或反之的诡异画面。
...
```
[Source: bundle.js L1371-L1446](https://github.com/camthink-ai/NeoMind-Dashboard-Components/blob/main/components/ne101_camera/bundle.js#L1371-L1446)，组件必须同时完成三个动作：(a) `AdvancedPanel` 用 `AI_EXT_IDS.indexOf(arr[i].id) >= 0` 过滤扩展列表（[L1490](https://github.com/camthink-ai/NeoMind-Dashboard-Components/blob/main/components/ne101_camera/bundle.js#L1490-L1490)），只显示白名单内的扩展；
...
测试矩阵验证「切换到 locate-anything-v2 时生成的 JS 包含 `nms_iou_threshold`、切换离开时这个字段消失」，防止 NMS 参数泄漏到不支持它的扩展（会导致 `image-analyzer-v2` 报「未知参数」错误）。
