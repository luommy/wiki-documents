...
### 新增 (Added)
- **NE301 PIR**：支持通过 Web 端灵活配置智能型 PIR 传感器的灵敏度、盲区时间等参数。
- **NE301 RTMP**：新增 CLI 指令支持 Video Hub 模式调试以及 Web 端 RTMP 推流功能。
- **NE301 PoE**：以太网供电支持，包含静态 IP/DHCP 配置及详细的状态监测。
- **案例应用**: NE301 连锁餐厅捕虫箱监测。


### 修正 (Fixed)
- **NE301固件功能**：修复远程唤醒开启后无法进入休眠问题。
