...
Don't want to build the image yourself? Download the prebuilt image [person-detection.tar](https://resources.camthink.ai/wiki/img/neoeyes-ne503-series/application-guide/app-development/person-detection/person-detection.tar) (this is the `image.tar`), pair it with the `app.yaml` from §2.2, and follow §5 to deploy to the device.
:::
