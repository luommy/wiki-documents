...
## 4.1 The Three-Layer Data Contract Model
...
## 4.2 Device Telemetry: MQTT Topics and WebSocket Messages
...
## 4.3 Four Extension Response Normalizers
...
## 4.4 Virtual Metrics Output Contract
...
## 4.5 The JSON-String Parsing Pitfall
...
## 4.6 ROI Overlap Detection Algorithm
...
## 4.7 WS-Priority + REST Backfill Dual Channel
...
- [5 Frontend Consume](./5-frontend-consume.md) (MVP) — How the component reads the normalized detections, colors them by class using `classColor`'s golden-angle HSV rotation, and maps bbox from 0-1 normalized coordinates to Canvas pixel coordinates (the non-linear scaling of `objectFit: contain`).
